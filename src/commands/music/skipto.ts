...
const meta = new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
        option.setName('tracknumber').setDescription('The track number to skip to.').setRequired(true).setMinValue(1)
    )
...
export default command(meta, async ({ client, interaction }) => {
    ...
    if (!nextTrack)
        return interaction.reply(Reply.error(`The track number specified must be less than ${queue.tracks.size}.`))
