...
const meta = new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
        option.setName('page').setDescription('The page number to display.').setRequired(false).setMinValue(1)
    )
...
export default command(meta, async ({ client, interaction }) => {
    ...
    if (!tracks[0])
        return interaction.reply(
            Reply.error('There are no songs in the queue. Use `/info` to view the currently playing song.')
        )
    ...
    const tracksString = tracks
        .slice(page * 10, page * 10 + 11)
        .map((song, i) => {
            return `**${(page * 10 + (i + 1)).toString().padStart(2, '0')}**. [${song.title}](${song.url}) (<@${
                song.requestedBy!.id
            }>, \`${song.duration}\`)`
        })
        .join('\n')
    ...
    await interaction.reply(
        Reply.info(
            `**Current Track:** [${queue.currentTrack!.title}](${
