...
const meta = new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
        option.setName('query').setDescription('Search for a song or playlist.').setRequired(true)
    )
...
export default command(meta, async ({ client, interaction }) => {
    ...
    if (botChannel && voiceChannel !== botChannel)
        return interaction.reply(Reply.error('You need to be in the same voice channel as me to use this command.'))
