...
export default command(meta, async ({ interaction }) => {
    ...
    if (!member.voice.channel)
        return interaction.reply(Reply.error('You must be in a voice channel to use this command.'))
    if (voiceChannel !== botChannel?.channelId)
        return interaction.reply(Reply.error('You must be in the same voice channel as the bot to use this command.'))
